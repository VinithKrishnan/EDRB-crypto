...
LIBSODIUM_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__),"." , "ed25519", "libsodium.so"))
...
class Point:
    ...
    @property
    def x(self):
        return recover_x(self.y, self.sign)
...
class Scalar:
    ...
    @staticmethod
    def random_seeded(rseed) -> "Scalar":
        random.seed(rseed)
        return Scalar(random.randint(0,GROUP_ORDER))
...
def keygen(rseed):
    ...
    """
    secret_key = Scalar.random_seeded(rseed)
    public_key = Point.B * secret_key
    return secret_key, public_key
# keygen(3)
def main():
    n = len(sys.argv)
    if n<3:
        print("Enter 2 arguments(both ints) 'NUM_KEYPAIRS SEED'")
        sys.exit()
    pkf = open("pubkey.txt","w")
    try:
        shutil.rmtree("edkeys")  
    except:
        pass
    os.mkdir("edkeys")
    num = int(sys.argv[1])
    ...
    sklist = []
    for i in range(num):
        sk,pk = keygen(int(sys.argv[2])+i)
        pklist.append(pk)
        sklist.append(sk)
        skf = open("edkeys/"+"key"+str(i)+".txt","w")
        skf.write(str(pklist[i].x)+":"+str(pklist[i].y)+"\n")
        skf.write(str(int(sklist[i])))
        skf.close()
    for pk in pklist:
        pkf.write(str(pk.x)+":"+str(pk.y)+"\n")
    pkf.close()
        # print(int(sk))
        # print(pk.x,pk.y)
